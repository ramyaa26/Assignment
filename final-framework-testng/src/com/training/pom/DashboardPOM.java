...
public class DashboardPOM {
	...
	@FindBy(xpath = "//span[text()='Sales']/following::a[text()='Orders'][1]") // indexing
																				// is
																				// used
																				// because
																				// 3
	...
	@FindBy(xpath = "//span[text()='Sales']/following::a[text()='Recurring Orders']") // indexing
																						// is
																						// used
																						// because
																						// 3
	...
	@FindBy(xpath = "//div[@class='table-responsive']//tbody/tr/td/a[1]") // list
																			// has
																			// all
																			// view
																			// buttons
	...
	@FindBy(xpath = "//div[@class='table-responsive']//tbody/tr/td/a[2]") // list
																			// has
																			// all
																			// edit
																			// buttons
	...
	@FindBy(xpath = "//div/button[@id='button-payment-address']")
	private WebElement continue_btn_3;
	
	@FindBy(xpath = "//div/button[@id='button-shipping-address']")
	...
	private List<WebElement> table_head;
	
	@FindBy(xpath="//table[1]//tbody[@id='cart']")
	private WebElement table_editOrder;
	
	@FindBy(xpath="//tbody[@id='cart']/tr/td[1]")
	private List<WebElement> table_body_products;
	
	@FindBy(xpath="//tbody[@id='cart']/tr/td[2]")
	private List<WebElement> table_body_model;
	
	@FindBy(xpath = "//div/input[@name='product']")
	private WebElement chooseproduct;

	@FindBy(xpath = "//div/ul[@class='dropdown-menu']/li/a")
	private List<WebElement> chooseproduct_list;

	@FindBy(xpath = "//div/input[@name='quantity']")
	private WebElement quantity;

	@FindBy(xpath = "//div/button[@id='button-product-add']")
	...
	@FindBy(xpath="//div/button[@id='button-save']")
	private WebElement save_btn;
	
	@FindBy(xpath="//div/button[@id='button-save']")
	private WebElement productnotavailable_alert;
	
	@FindBy(xpath = "//ul/li/a/i[@class='fa fa-user fa-fw']")
	private WebElement customers_icon;
	
	@FindBy(xpath="//li[@id='customer']//ul/li/a[text()='Customers']")
	...
	public void clickProductContinue_Btn(){
		this.continue_btn_2.click();
	}
	
	public void clickPaymentContinue_Btn(){
		this.continue_btn_3.click();
	}
	
	public void clickShippingContinue_Btn(){
		this.continue_btn_4.click();
	}
	...
	public void verifyTaxReportTable_Values() {
		...
		String total = this.report_Results.get(4).getText().substring(1); // substring
																			// is
																			// used
																			// to
																			// remove
		...
		if (start_Date.equals(end_Date)) {
			System.out.println("For the date " + start_Date + " total tax paid is displayed as " + total + " for "
					+ no_Orders + " orders at " + tax_Title);
		} else {
			System.out.println("For dates between " + start_Date + " and " + end_Date
					+ " total tax paid is displayed as " + total + " for " + no_Orders + " orders at " + tax_Title);
		}
	...
	public void verifyShippingReportTable_Values() {
		...
		String total = this.report_Results.get(4).getText().substring(1); // substring
																			...
																			// total
		if (start_Date.equals(end_Date)) {
			System.out.println("For the date " + start_Date + " shipping total is displayed as " + total + " for "
					+ no_Orders + " orders at " + shipping_Title);
		} else {
			System.out
					.println("For dates between " + start_Date + " and " + end_Date + " shipping total is displayed as "
							+ total + " for " + no_Orders + " orders at " + shipping_Title);
		}
	...
	public void clickEditIcon() {
		...
		if (verifyOrderListTable()) {
			...
			System.out.println("Order List table has no data");
		}
	}
	...
	public void chooseProduct(String productname, String qty) {
		...
		//action.click(this.chooseproduct);
		
		JavascriptExecutor js = (JavascriptExecutor) driver;  
		//js.executeScript("arguments[0].click();",chooseproduct);

		js.executeScript("arguments[0].value='"+productname+"'",chooseproduct);
		
		this.chooseproduct.sendKeys(Keys.TAB);
		
	    
		
		for (int i=0;i<this.chooseproduct_list.size();i++)
		{
			System.out.println(chooseproduct_list.get(i).getText());
			if((chooseproduct_list.get(i).getText()).equals(productname))
			{
				chooseproduct_list.get(i).click();
				break;
			}
		}
	...
	public List<String> getOrderListTableValues()
	{
		data = null;
		
		List<WebElement> rows = this.table_editOrder.findElements(By.tagName("tr"));
		...
		for(int rnum=0;rnum<rows.size();rnum++)
		{
			...
			String temp = columns.get(cnum).getText();
			data.add(temp);
			}
		}
		return data;
